...
URL 入力から Renderer ヘルスチェック、HTTPS 取得、frameset 解析、描画、リンク遷移までの統合シーケンスです。
...
sequenceDiagram
  ...
  participant IPC as dispatch_ipc
  participant PM as ProcessHostManager
  ...
  IPC->>PM: healthcheck()
  alt renderer healthy
    PM-->>IPC: ok
  else renderer exited
    PM->>PM: restart()
    PM-->>IPC: renderer_recovering(retryable)
    IPC-->>UI: AppError(code=renderer_recovering)
    UI-->>User: 「復旧中。再試行してください」
  end
