...
加えて E7-T2 JS runtime smoke として、`adapter_cli verify-event-loop` を使った最小スクリプト接続を検証する。

- `e7_t2_static.html`: `DOMContentLoaded` 相当フローで初期描画の DOM 更新が反映されること。
- `e7_t2_spa.html`: `input`/`change`/`click` dispatch 経路で DOM 更新が反映されること。
