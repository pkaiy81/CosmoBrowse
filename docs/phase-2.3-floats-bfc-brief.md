...
>
> **除外済みの容疑**(いずれも計測で否定):
> ① 継承の失敗 — 継承は届いている(552 ブロック)。
> ② BFC 境界 — 無視させても 20→17 とほぼ不変。
> ③ `translated()` の座標変換ミス — x を変換せず祖先の `content_width` を使い続けていたのは
>    **実際にバグだったので修正した**(テスト付き)が、20 断片は **20 のまま変わらなかった**。
>
> **次に測るべき**: 「短縮されなかった行」を1本選び、その行を出した `layout_inline_items` 呼び出しで
> `band()` に渡っている y と返り値を直接ログする。ブロック単位の集計では届かないところまで来ている。
> `FloatContext` 側は上記③でひととおり正しくなったので、疑うべきは
> **行の y がブロック内相対で正しく渡っているか**(IFC は 0 起点、context は祖先起点)。
>
