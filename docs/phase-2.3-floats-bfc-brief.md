...
>
> **⚠ 行短縮の到達範囲**: reftest では**確実に効く**(float の右端 x=110 から行が始まることを実測)。
> しかし **Wikipedia の記事段落には届かない** — float の箱(x 580..1000, y 975..1183)に
> 記事テキストの断片が 20 個食い込んでいる。原因は未特定。
> **BFC 境界で継承が止まるせいではない**(境界を無視させても 20→17 とほぼ不変)。
>
> つまり Wikipedia の改善(記事冒頭が全幅画像ブロックの下に押し下げられなくなった)は
> **float が out-of-flow + shrink-to-fit になった効果**であって、行短縮の効果ではない。
> 次はここから: 「なぜ `ancestor_float_context` が Wikipedia の段落に届かないのか」を
> `COSMO_DEBUG_FLOATS` 相当の計測で特定する。
>
