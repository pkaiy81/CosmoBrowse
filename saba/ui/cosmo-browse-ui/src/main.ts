...
let reloadBtnEl: HTMLButtonElement | null;
...
function applyNavigationState(nav: NavigationState) {
  ...
  if (reloadBtnEl) {
    reloadBtnEl.disabled = !nav.can_reload;
  }
}
...
async function refreshNavigationState() {
  try {
    ...
    if (reloadBtnEl) reloadBtnEl.disabled = true;
  }
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  reloadBtnEl = document.querySelector("#reload-btn");
