...
> - ✅ **既定を Boa に切替 (`7b08aed`, 2026-07-24)** — GUI ヘッドレスで検証済み:
>   reftest 11/11 と凍結フィクスチャ(HN/MDN/Wikipedia)が玩具 vs Boa で**画素完全一致**
>   (golden 再ベースライン不要)。JS デモ(textContent/createElement/appendChild/classList/
>   querySelectorAll)は Boa のみ正しく描画。`use_boa_engine()` は既定 ON、玩具に戻すには
>   `COSMO_USE_BOA=0` または `COSMO_LEGACY_JS=1`。**js_dom reftest** を追加(Boa で pass、
...
> - ✅ **フォーム入力 + `:focus` (PR #93, 2026-08-28)** — **フィールドに文字が打てる**ように。
>   クリックでフォーカス → キー入力で値が変わる → ページが `input` イベントを受け取る。
>   値の置き場は要素ごとに違う: `<input>` は `value` 属性、`<textarea>` は**子テキスト**
>   (HTML LS §4.10.11)。どちらも**レイアウトが既に描いている場所**なので描画側の変更はゼロ、
>   他の DOM 変異と同じ経路で再レイアウトされる。キー入力を受けるのはテキスト系 type のみ
>   (`submit`/`checkbox` はクリック対象、type 省略は text)。
>   **`:focus` / `:focus-within`** も解禁(どちらも `Never` に落ちていた)。`:hover` と同じく
>   レンダラがチェーンを発行し、`:focus` は先頭・`:focus-within` は全体にマッチ。
>   再レイアウトは `uses_focus()` のページのみ。`:focus-visible` は据え置き
>   (マウスクリックでは偽なので、`:focus` 扱いにすると実ブラウザが描かないリングが出る)。
>   `COSMO_HEADLESS_TYPE` でスクショ検証可能。
>   **教訓**: textarea で「ユニットテストは全通過なのに GUI では1文字ごとに改行」が発生。
>   値の読み取りが `collect_text` に**要素自身**を渡しており、この関数は**次の兄弟も辿る**ため
>   `</textarea>` 直後の改行ノードが値に混ざっていた。テストのフィクスチャが綺麗すぎた
>   (先頭空白なし・後続要素なし)のが見逃した理由。**GUI で実際に動かすまで分からない類**。
>   **残**: キャレット描画/カーソル位置(末尾追加・末尾削除のみ)、選択、矢印キー、`:active`。
